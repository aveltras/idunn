...
void idunn_physics_body_init(void *system, idunn_physics_body_settings *settings) {
  static_cast<Physics::System *>(system)->create(settings);
}
...
Physics::System::System(Physics *physics, idunn_physics_system_config *config)
    : physics(physics),
      ...
      pActiveBodyCount(config->pActiveBodyCount),
      ppActiveBodies(config->ppActiveBodies),
...
auto Physics::System::create(idunn_physics_body_settings *settings) -> void {
  ...
  auto bodyID = JPH::BodyID(settings->bodyID);
  bodyInterface.CreateBodyWithID(bodyID, bodyCreationSettings);
...
auto Physics::System::update() -> void {
  ...
  *pContactRemoved = contactRemoved.data();
  ...
  activeRawBodyIds.clear();
  *pActiveBodyCount = system.GetNumActiveBodies(JPH::EBodyType::RigidBody);

  if (*pActiveBodyCount > 0) {
    auto *transforms = *transformData;
    JPH::BodyInterface &bodyInterface = system.GetBodyInterface();
    const auto *activeBodyIds = system.GetActiveBodiesUnsafe(JPH::EBodyType::RigidBody);

    for (auto bodyIdx = 0; std::cmp_less(bodyIdx, *pActiveBodyCount); bodyIdx++) {
      auto bodyID = activeBodyIds[bodyIdx];
      auto bodyTransform = bodyInterface.GetCenterOfMassTransform(bodyID);
      activeRawBodyIds.push_back(bodyID.GetIndexAndSequenceNumber());
      bodyTransform.StoreFloat4x4((JPH::Float4 *)&transforms[bodyID.GetIndexAndSequenceNumber()]);
    }

    *ppActiveBodies = activeRawBodyIds.data();
  }
}
